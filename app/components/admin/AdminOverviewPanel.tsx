  ...
  return (
    ...
    >
        ...
        {kpiCards.map((card, i) => {
          ...
          return (
            <motion.div
              key={i}
              variants={fadeInUp}
              className={`relative bg-white rounded-2xl border border-slate-200/80 border-l-4 ${card.borderAccent} p-6 shadow-sm hover:shadow-md transition-shadow`}
            >
              <div className="flex items-start justify-between mb-4">
                ...
                {card.action ? (
                  <button
                    onClick={card.action}
                    className="text-sm font-semibold text-cyan-600 hover:text-cyan-700 flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    {card.actionLabel}{" "}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <motion.div
          variants={fadeInUp}
          className="xl:col-span-2 bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm"
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-2">
              {topDestinations.slice(0, 3).map((dest, i) => (
                <span
                  key={i}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200/60 text-xs font-semibold text-slate-700"
                >
                  <span
          ...
          {/* Bar Chart */}
          <div className="grid grid-cols-12 gap-3 items-end h-72 pt-4 pb-3 border-b border-slate-100">
            {revenueData.map((bar, i) => (
              <div
                key={i}
                className="flex flex-col items-center gap-2 h-full justify-end group"
              >
                <span className="text-[11px] font-bold text-cyan-700 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  {bar.amount}
                </span>
                <motion.div
                  initial={{ height: 0 }}
                  ...
                  className="w-full bg-gradient-to-t from-slate-800 to-slate-600 rounded-t-lg group-hover:from-cyan-600 group-hover:to-cyan-400 transition-all duration-300 cursor-pointer shadow-sm"
                />
        ...
        >
          ...
          <button
            ...
            className="w-full mt-6 py-3 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-semibold border border-slate-200/60 flex items-center justify-center gap-2 transition-colors cursor-pointer"
          >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {recentActivity.map((act, i) => (
            <motion.div
              ...
              className="flex items-start gap-4 p-5 rounded-xl bg-slate-50/80 border border-slate-100 hover:bg-slate-50 transition-colors"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-base font-extrabold text-cyan-700 shrink-0">
                    {act.amount}
                  </span>
                </div>
                <p className="text-sm text-slate-600 mt-0.5">{act.action}</p>
