...
export default function NavbarClient({ user, isAuth, isAdmin = false }: NavbarClientProps) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden items-center gap-4 lg:flex">
              {isAuth ? (
                ...
                <div className="flex items-center gap-4">
                  <NextLink href="/api/auth/login" className="group relative h-10 w-24 overflow-hidden rounded-full border border-white/20 bg-white/5 hover:bg-white/10 transition-colors block">
                  ...
                  <NextLink href="/api/auth/register" className="group relative h-10 w-32 overflow-hidden rounded-full bg-cyan-600 hover:bg-cyan-500 transition-colors shadow-lg shadow-cyan-900/20 block">
                    <div className="relative h-full w-full">
                      <span className="absolute inset-0 flex items-center justify-center transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:-translate-y-full">
                        {t('register')}
                      </span>
              ...
              {isAuth ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {isAdmin && (
                      <Link href="/admin" className="col-span-2 p-4 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center gap-2 hover:bg-cyan-500/20 transition-colors">
                        <PlusCircle className="w-5 h-5 text-cyan-400" />
                        <span className="text-cyan-400 text-sm font-semibold">Admin Console</span>
                      </Link>
                    )}
                ...
                <div className="grid grid-cols-1 gap-4">
                  <NextLink href="/api/auth/login" className="w-full py-4 rounded-2xl bg-white/5 text-white text-center font-bold border border-white/10 block">
                    {t('login')}
                  </NextLink>
                  <NextLink href="/api/auth/register" className="w-full py-4 rounded-2xl bg-cyan-600 text-white text-center font-bold block">
                    {t('register')}
                  </NextLink>
                </div>
              )}
